...
int envoie_recois_message(char *pathname) {
  ...
  char message[500] = "";
  if(strcmp(typeMessage, "stop") == 0) {
    return 1;
  } else if(strcmp(typeMessage, "couleurs") == 0 || strcmp(typeMessage, "balises") == 0 || strcmp(typeMessage, "plot") == 0) {
    analyse(pathname, message);
    sprintf(data, "{\"id\":\"%s\",\"code\":\"%s\",\"valeurs\":[%s]}", id, typeMessage, message);
  } else {
...
int main(int argc, char **argv) {
  ...
  */
  while(envoie_recois_message(argv[1]) == 0)
  close(socketfd);
}
